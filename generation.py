...
def _eval_epoch(sess, epoch, mode):
    seq, diff, references, hypotheses = [], [], [], []
    ...
    with open('./output/refs_wash.txt', 'w') as f1, open('./output/hyps_wash.txt', 'w') as f2,\
            open('./output/srcs_wash.txt', 'w') as f3, open('./output/seqs_wash.txt', 'w') as f4:
        while True:
            ...
            try:
                ...
                feed_dict = {
                    ...
                    seq_input_ids: op[0]['seq_input_ids'],
                    seq_segment_ids: op[0]['seq_segment_ids'],
                    tx.global_mode(): tf.estimator.ModeKeys.PREDICT
                }
                fetches_ = sess.run(fetches, feed_dict=feed_dict)
                labels = op[0]['tgt_labels']
                diffs = op[0]['src_input_ids']
                seqs = op[0]['seq_input_ids']
                ...
                seq.extend(d.tolist() for d in seqs)
        ...
        seq = utils.list_strip_eos(seq, eos_token_id)
        ...
        for example in range(len(references)):
            ...
            cwords = tokenizer.convert_ids_to_tokens(seq[example])
