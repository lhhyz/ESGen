...
class InputFeatures():
    ...
    def __init__(self, src_input_ids, src_input_mask, src_segment_ids, seq_input_ids, seq_input_mask, seq_segment_ids,
        ...
        self.seq_input_ids = seq_input_ids
        self.seq_input_mask = seq_input_mask
        self.seq_segment_ids = seq_segment_ids
...
class CNNDailymail(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        return self._create_examples(
            ...
            self._read_file(os.path.join(data_dir, "train_seq.txt")),
    ...
    def get_dev_examples(self, data_dir):
        ...
        return self._create_examples(
            ...
            self._read_file(os.path.join(data_dir, "eval_seq.txt")),
    ...
    def get_test_examples(self, data_dir):
        ...
        return self._create_examples(
            ...
            self._read_file(os.path.join(data_dir, "test_seq.txt")),
            self._read_file(os.path.join(data_dir, "test_summ.txt")),
            "test")

    def _create_examples(self, src_lines, seq_lines, tgt_lines, set_type):
        examples = []
        for i, data in enumerate(zip(src_lines, seq_lines, tgt_lines)):
            guid = "%s-%s" % (set_type, i)
            if set_type == "test" and i == 0:
                ...
                seq_lines = tokenization.convert_to_unicode(data[1][0])
                tgt_lines = tokenization.convert_to_unicode(data[2][0])
                examples.append(InputExample(guid=guid, text_a=src_lines, text_c=seq_lines, text_b=tgt_lines))
        return examples


def file_based_convert_examples_to_features(
        examples, max_seq_length_src, max_seq_length_seq, max_seq_length_tgt, tokenizer, output_file):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        feature = convert_single_example(ex_index, example,
                                         max_seq_length_src, max_seq_length_seq, max_seq_length_tgt, tokenizer)
        ...
        features["seq_input_ids"] = create_int_feature(feature.seq_input_ids)
        features["seq_input_mask"] = create_int_feature(feature.seq_input_mask)
        features["seq_segment_ids"] = create_int_feature(feature.seq_segment_ids)
...
def convert_single_example(ex_index, example, max_seq_length_src, max_seq_length_seq, max_seq_length_tgt,
                           tokenizer):
    ...
    tokens_c = tokenizer.tokenize(example.seq_txt)
    ...
    if len(tokens_c) > max_seq_length_seq - 2:
        tokens_c = tokens_c[0:(max_seq_length_seq - 2)] #原本这里是tokens_a
    ...
    tokens_seq = []
    segment_ids_seq = []
    tokens_seq.append("[CLS]")
    segment_ids_seq.append(0)
    for token in tokens_c:
        tokens_seq.append(token)
        segment_ids_seq.append(0)
    tokens_seq.append("[SEP]")
    segment_ids_seq.append(0)
    ...
    input_ids_seq = tokenizer.convert_tokens_to_ids(tokens_seq)
    ...
    input_mask_seq = [1] * len(input_ids_seq)
    ...
    while len(input_ids_seq) < max_seq_length_seq:
        input_ids_seq.append(0)
        input_mask_seq.append(0)
        segment_ids_seq.append(0)
    ...
    feature = InputFeatures(src_input_ids=input_ids_src, src_input_mask=input_mask_src, src_segment_ids=segment_ids_src,
                            seq_input_ids=input_ids_seq, seq_input_mask=input_mask_seq, seq_segment_ids=segment_ids_seq,
                            tgt_input_ids=input_ids_tgt, tgt_input_mask=input_mask_tgt, tgt_labels=labels_tgt)

    return feature


def file_based_input_fn_builder(input_file, max_seq_length_src, max_seq_length_seq, max_seq_length_tgt, is_training,
    ...
    name_to_features = {
        ...
        "seq_input_ids": tf.FixedLenFeature([max_seq_length_seq], tf.int64),
        "seq_input_mask": tf.FixedLenFeature([max_seq_length_seq], tf.int64),
        "seq_segment_ids": tf.FixedLenFeature([max_seq_length_seq], tf.int64),
...
def get_dataset(processor,
    ...
    if mode == 'train':
        ...
        file_based_convert_examples_to_features(
            train_examples, max_seq_length_src, max_seq_length_seq, max_seq_length_tgt,
            tokenizer, train_file)
        dataset = file_based_input_fn_builder(
            input_file=train_file,
            max_seq_length_src=max_seq_length_src,
            max_seq_length_seq=max_seq_length_seq,
        ...
        file_based_convert_examples_to_features(
            eval_examples, max_seq_length_src, max_seq_length_seq, max_seq_length_tgt,
            tokenizer, eval_file)
        dataset = file_based_input_fn_builder(
        ...
        file_based_convert_examples_to_features(
            test_examples, max_seq_length_src, max_seq_length_seq, max_seq_length_tgt,
            tokenizer, test_file)
        dataset = file_based_input_fn_builder(
...
if __name__ == "__main__":
    ...
    train_dataset = get_dataset(processor, tokenizer, data_dir, max_seq_length_src, max_seq_length_seq,
                                max_seq_length_tgt, batch_size,
                                'train', data_dir)
    eval_dataset = get_dataset(processor, tokenizer, data_dir, max_seq_length_src, max_seq_length_seq,
                               max_seq_length_tgt, eval_batch_size,
                               'eval', data_dir)
    test_dataset = get_dataset(processor, tokenizer, data_dir, max_seq_length_src, max_seq_length_seq,
