...
seq_input_ids = tf.placeholder(tf.int64, shape=(None, None))
seq_segment_ids = tf.placeholder(tf.int64, shape=(None, None))
...
seq_input_length = tf.reduce_sum(1 - tf.to_int32(tf.equal(seq_input_ids, 0)), axis=1)
...
with tf.variable_scope('gru'):
    ...
    word_embeds = embedder(seq_input_ids)

    # Creates segment embeddings for each type of tokens.
    segment_embedder = tx.modules.WordEmbedder(
        vocab_size=bert_config.type_vocab_size,
        hparams=bert_config.segment_embed)
    segment_embeds = segment_embedder(seq_segment_ids)
